...
import { createScreenMode, type ScreenMode } from './view/shared/screen-mode.js';
import { PLAYER_ENTRANT, profileOf } from './view/shared/variants.js';

export interface RacingModuleOptions {
  /**
   * 画面モード（実装計画 11-7）。**アプリと共有する** — レンダラーは
   * `crtOverride()` を毎フレーム読み、モジュールはキーで切り替えて現在値を表示する。
   * 省略時はモジュールが自前で持つ（テストと単体起動のため）。
   */
 ...
 */
export function createRacingModule(options: RacingModuleOptions = {}): GameModule {
  const screenMode = options.screenMode ?? createScreenMode();
  return {
    ...
    async create(context: GameContext): Promise<GameInstance> {
      ...
      return {
        fixedUpdate() {
          ...
          // 画面モード（11-7）。世代切替と同じでどの画面でも効く。
          // 絵を変えるのはレンダラーが毎フレーム読む `crtOverride()` のほうで、
          // ここがやるのは状態を倒すことだけ
          if (input.toggleFlat.pressed) screenMode.toggleFlatDisplay();
          if (input.toggleMoire.pressed) screenMode.toggleMoire();
