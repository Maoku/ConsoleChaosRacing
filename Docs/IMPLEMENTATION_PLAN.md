...
| # | 項目 | 対象世代 | 主に触るファイル | アセット再生成 |
| --- | --- | --- | --- | --- |
| 8-1 | 直線でステアフレームを出さない | 1・2 | `view/shared/car-sprite.ts` | 無 |
| 8-2 | アフィン面の解像度を SFC の BG スペックへ落とす | 2 | `road-surface.ts` / `tools/build-road-texture.mjs` | **有** |
| 8-3 | タコメーターを左下に追加 | 3・4 | `view/shared/tachometer.ts`（新規）/ `hud.ts` | **有** |
| 8-4 | カメラを車体へ近づける | 3・4 | `view/shared/camera.ts` | 無 |
| 8-5 | 視点切り替え（フロントガラス／内装） | 3・4 | `camera.ts` / `gen3-ps1.ts` / `gen4-ps2.ts` / `bindings.ts` | **有**（内装のみ） |
| 8-6 | 背景オブジェクト | 全 | `view/shared/scenery.ts`（新規）/ `tools/build-track-mesh.mjs` | **有** |
