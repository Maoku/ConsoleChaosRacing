...
- 更新: 2026-08-14（**フェーズ 9 を新設し 9-1 コースアウトからの復帰を実装** — 壁が「位置の固定 ＋ 向きに関わらないヨー減衰 ＋ 速度の頭打ち」の 3 つを同時にやると、内向きの舵でヨーが育つ前に潰され、グリップ超過ぶんの横滑りが外向きに勝って**壁から永久に離れられない**こと・罰は速度に掛け舵には掛けないこと・路外の罰は縁からの深さで連続にすべきこと・舵の限界を `steeringLimits()` へ集約してシムと AI が同じ関数を通ること）
...
- 更新: 2026-08-19（**フェーズ 10 を新設し敵車の速さのバランス改修を実装** — `Docs/BALANCE.md` の要求に対する計画は `Docs/BALANCE_PLAN.md`。確定した事実: 直線が 434 m しかないこのコースでは `MAX_SPEED` は終速の上限として一度も効かず、**定数を 0.95 倍しても走行中の最高速度の比は 0.971 にしかならない**こと・`MAX_SPEED` と `ACCEL` を同じ倍率で落とせば速度そのものが比例すること・目標タイムへの追従は残り距離ではなく**基準走行の時間表**で測らないと周回内の位置で要求が振れること）
- 更新: 2026-09-09（**フェーズ 12 を新設し第3・第4世代の車モデルを差し替え** — 要求は `Docs/REPLACE_DATA.md`、計画は `Docs/REPLACE_DATA_PLAN.md`。確定した事実: 実行時の `TransformCommand` が `rotationY` と等方 scale しか持たない以上、**姿勢・原点・寸法の正規化は変換器へ焼き込むしかない**こと（node の TRS へ逃がすと accessor の min/max が姿勢と食い違い `CAR_MODELS.bounds` の意味が壊れる）・車幅で合わせて**相似に縮めると全長が衝突判定より 0.4 m 短くなる**ので前後だけ別倍率で伸ばす必要があること・素の塗装が明るい素材では `PAINT_TARGET` 0.82 が塗装面の 6 割を白飛びさせること・低ポリのメッシュは三角形分割自体が左右非対称なので対称性は頂点どうしではなく**面までの距離**で測る必要があること）
- 更新: 2026-09-09（**12-7 で第3・第4世代にテールランプとタイヤの回転を追加** — 確定した事実: `TransformCommand` に `rotationY` しか無いので車軸まわりの回転は**位相を焼いた GLB を差し替える**しか無いこと・スキン経路はマテリアルを読まない（`uEnvironmentStrength: 0`・`uAmbient` 0.45 固定）ので第4世代の映り込みと両立しないこと・`MaterialCommand.emissiveTexture` は型にあるだけで**レンダラーが読んでいない**こと・変換元の車はタイヤが独立した連結成分として入っているので切り離しても車体に穴が開かないこと・正規化の異方倍率を戻さずに回すと車輪の輪郭が首を振ること（bounds のぶれ 0.0239 → 0.0110）・車の後ろ姿は平らではないので灯火の板は**列ごとに表面の X を測り直さないと車体からはみ出す**こと）
...
| 車ソース GLB | `data/gen3_car_tripo.glb`, `data/gen4_car_tripo.glb` | 変更禁止。旧 `gen{3,4}_car.glb` も残す（フェーズ 12 で差し替え） |
| 変換済み車モデル | `public/assets/gen{3,4}/models/car.glb` | **964 tri / 7,997 tri**、前方 `-X`。**skin・animation・material・image をすべて持たない**（変換で除去済み）。姿勢・原点・寸法の正規化は変換で焼き込む（フェーズ 12） |
| 車テクスチャ | `public/assets/gen{3,4}/textures/car_base_color.png` | 256² / 1024²。**変換元から焼き直せる**（フェーズ 12 以降）。実行時に読むのは `car_paint.png` のほう |
...
| 変換記録 | `public/assets/car-conversion.json` | `version` 2。SHA-256 / 正規化定数（`normalize`）/ 三角形数 / bounds を保持。**パスはリポジトリルート相対**（`data/...`, `public/assets/...`） |
...
- 入力の `data/*.glb` は**絶対に上書きしない**
- POSITION / NORMAL / TEXCOORD_0 / indices / 三角形数を保存する
- 未使用の normal・metallic/roughness 画像を除去、base color を外部テクスチャとして書き出す
- 単位行列 `node.matrix` を暗黙の TRS へ正規化する
...
> **フェーズ 12 で変換規則が変わった。** モデルを差し替えたことで「bounds を保存する」は
> 成り立たなくなり、**bounds は正規化の出力**になった（回転・中心合わせ・寸法合わせを
> 焼き込む）。`geometry.fingerprint` は記録から落とし、base color は変換元から焼き直す。
> 現行の規則は `data/README.md` にある。
...
**路外と壁**（9-1 で確定）。「路面の外は 1 段だけ悪い」という二値ではなく、**縁からの深さ**で連続的に扱う。

```
severity = offTrackSeverity(lateral, halfWidth)    0（路面上）… 1（縁から 3 m 以上）
減速     = OFF_TRACK_DRAG * severity               深いほど強く効く
グリップ = GRIP_ACCEL * (1 - (1-OFF_TRACK_GRIP) * severity)
戻り舵   = 路面の側へ切った舵にだけ GRIP_ACCEL * RECOVERY_GRIP を返す
壁       = 外向きの運動だけを吸う（位置は内側へ WALL_BOUNCE・外向きヨーだけ減衰・
           速度の罰は外向きの横速度に比例）
```

舵に効く限界は `steeringLimits()` 1 つに集約し、**`stepVehicle()` と AI が同じ関数を通る**。
`severity` が 0 のあいだはどの項も消えるので、**路面の上の走りは従来の式と 1 ビットも変わらない**
（`vehicle-recovery.spec.ts` が固定する）。
...
| 初回ロード | manifest が読む全アセット合計 **6.75 MB**（フェーズ 12 実測・96 ファイル）。プリロード完了まで進行度表示を出す |

内訳の大きいものは第4世代のコースメッシュ・環境マップ（0.70 MB）・第4世代の車モデル（0.36 MB）・第3世代のコースメッシュ・遠景の帯（0.35 MB）。**変換の入力として残している車の base color は実行時に読まない**（実行時に読むのは無彩色の `car_paint.png`）ので、フェーズ 12 で第4世代の base color が 1.26 → 1.84 MB へ増えたぶんは初回ロードに乗らない。フェーズ 12 の差し替えで初回ロードは 6.94 → 6.75 MB（−196 KB）になった — 車モデルが −240 KB、塗装テクスチャが +45 KB。
...
**三角形の実測**（走行中の 1 フレーム・フェーズ 12 の車で測り直したもの）: 第3世代は路面 9,894 tri ＋ 車 8 台 7,712 tri ＝ **17,606 tri**（予算 20,000 の内側）。第4世代は路面 25,916 tri ＋ 車 8 台 63,976 tri ＋ タイヤバリア 2,040 tri ＝ **91,932 tri** で、**予算を大きく超える**（フェーズ 11 時点の 136,900 tri からは減った）。20,000 tri はリリースノートが第3世代の ordering table の partition 性能（CPU 側の安定分割）を測った値であり、深度バッファがあってソートを一切しない第4世代には掛からない。
...
各フェーズの終わりに動く物ができる形で並べている。フェーズ 9 以降は、実装が一巡したあとに
要求が足されるたびに新設したもの（9 コースアウトからの復帰・10 バランス・11 機能改善・
12 車モデルの入れ替え）。
...
実画面で遊んで見つかった、**表現ではなく走りそのもの**の不足を扱うフェーズ。基準は §6.1 では
測れない（どの世代でも同じシムなので、4 世代で同じように起きる）。

| # | 項目 | 主に触るファイル | アセット再生成 | 状態 |
| --- | --- | --- | --- | --- |
| 9-1 | コースアウトからの復帰をしやすくする | `sim/vehicle.ts` / `sim/ai.ts` | 無 | ✅ |

#### 9-1 コースアウトからの復帰（全世代）

**現象**: 一度コースを外れて壁まで流れると、**全開で戻り舵を当てても永久に戻れない**。
`tests/vehicle-recovery.spec.ts` の条件（壁ぎわ・全開・フル戻り舵）を 30 秒回しても、
車は壁に貼り付いたまま `lateral` が 1 mm も内側へ動かなかった。

**原因は 3 つが噛み合ったもの**（どれ 1 つでも欠けると起きない）。

1. 壁が**位置を壁ちょうどへ留め**、**ヨーを向きに関わらず 0.3 倍**にしていた。内向きの舵で
   ヨーが育つ前に毎ティック潰されるので、横方向の速度 `speed * sin(yaw)` がほぼ 0 のまま
2. 路外のグリップ 0.45 倍のせいで、要求ヨー角速度がグリップ上限を超える。**その差は
   `slide` として外向きに効く** — 実測で内向き 0.03 m/s に対し外向き 0.23 m/s。
   **戻ろうとするほど外へ押される**
3. 速度が `WALL_SPEED`（14 m/s）で頭打ちになるうえ路外の抵抗が 9 m/s² 掛かるので、
   到達速度は 9.6 m/s。舵の効きは速度に比例する（`YAW_SPEED_REF`）ので二重に戻れない

**決定**: 罰は速度に掛け、舵には掛けない。速度の罰（＝路外が近道にならない保証）は
一切緩めずに、**向きを変える自由だけを返す**。

- **壁は外向きの運動だけを吸う** — 位置は壁より `WALL_BOUNCE`（0.4 m）内側へ戻し、
  ヨーは**外を向いているときだけ**減衰させ、速度の罰は外向きの横速度に比例させる
  （`WALL_BITE`）。掠っただけならほとんど削られず、正面から突っ込めば大きく削られる
- **戻り舵にだけグリップを返す**（`RECOVERY_GRIP` = 0.85）。路面の側へ切った舵のときだけ
- **路外では無理な舵を許さない**（`overshoot` = 1）。要求がグリップ上限を超えないので
  原因 2 の外向きの `slide` が出ない
- **罰を深さで段階化**（`offTrackSeverity`）。縁を割った直後は 0.3 倍から始まり、
  3 m で 1 倍に達する。浅いミスは浅く罰する
- 舵の限界は `steeringLimits()` に集約し、**シムと AI が同じ関数を通る**

`WALL_SPEED` は廃した（比例する罰が置き換える）。`severity` が 0 のあいだは追加項がすべて
消えるので、**路面の上の走りは変わっていない** — AI だけで 3 周走らせたリプレイは、
順位・ベストラップ・完走ティックまで変更前と完全に一致する（AI は路外へ出ないため）。

**検証**: `tests/vehicle-recovery.spec.ts`（10 件）。復帰時間・壁の扱い・罰の単調性・
「路外が近道にならない」・「路面上では手心が一切掛からない」を固定する。コース 1 周を
20 m 刻みで走査して、**戻れない地点が 1 つも無い**ことも含める。実測は
`Docs/QUALITY_REVIEW.md` §10。

---

### フェーズ 10 — ゲームバランス（敵車の速さ）✅ 実装済み

要求は `Docs/BALANCE.md`、計画と実測は `Docs/BALANCE_PLAN.md`。実測は
`Docs/QUALITY_REVIEW.md` §11。**触るのは `sim/` の 3 ファイルだけ**で、`view/` `audio/`
`flow/` には 1 行も入っていない（シミュレーションは 1 つ、が守られていることの確認でもある）。

| # | 項目 | 主に触るファイル | アセット再生成 | 状態 |
| --- | --- | --- | --- | --- |
| 10-1 | 敵車の速度モデルに車ごとの倍率を入れる | `sim/state.ts` / `sim/vehicle.ts` / `sim/ai.ts` | 無 | ✅ |
| 10-2 | 速度スケールとペースの較正ツール | `tools/measure-pace.mjs` | 無 | ✅ |
| 10-3 | 車ごとの目標タイムと難易度テーブル | `sim/state.ts` | 無 | ✅ |
| 10-4 | 目標タイムを達成させるペース制御 | `sim/ai.ts` | 無 | ✅ |
| 10-5 | アトラクトデモの自機に中位の目標を与える | `sim/state.ts` | 無 | ✅ |
| 10-6 | 難易度のアンカーを較正ツールへ載せる | `tools/measure-pace.mjs` | 無 | ✅（人のプレイテストは残件） |

#### 10-1 最高速度は自機の 0.95（要求 R-1）

**定数を 0.95 倍しても速度差はほとんど出ない。** このコースの最長直線は 434 m しかなく、
0 → 74.1 m/s には 1,132 m 要る。つまり `MAX_SPEED` は「終速の上限」としてこのコースでは
一度も効かず、**加速の鈍り方としてしか効かない**（実測比 0.971）。

**決定**: `MAX_SPEED` と `ACCEL` の**両方**へ倍率 `BALANCE.SPEED_SCALE` を掛ける。
`dv/dt = kA(1 − v/kV)` は `v → kv` で元の式に一致するので、**直線のどの瞬間でも速度が
ちょうど k 倍**の車になる。実測比が 0.950 になる値は 0.933（二分探索）。

**コードの 0.933 と要求の 0.95 は別物である。** 0.95 は定数ではなく実測される比であり、
定数コメント・較正ツールの出力・テストの 3 箇所で主語を「実測比」に揃えてある。
コース形状を変えたら `npm run measure:pace` で測り直す（README に運用を記載）。

#### 10-3 / 10-4 目標タイムとペース制御（要求 R-2）

敵車 7 台へグリッド順に目標**総レース時間**を配り（トップと最下位で 15 秒差・スタート時に
0〜+2 秒のばらつき）、`driveAi()` が毎ティック**残りの道のりと残り時間から必要な平均ラップを
出してペース倍率へ写す**閉ループでそれを達成する。上限 1.0 が「ゴム紐にしない」に対応する。

**残りの道のりは距離ではなく時間で測る。** 計画どおり残り距離で測ると、ヘアピン 19 m/s と
直線 63 m/s の差で**要求ラップが周回内の位置に応じて ±2 秒振れ**、AI が直線で理由もなく
緩んではヘアピンで焦り、最後に帳尻を落とした（目標との差 −0.00 〜 +2.28 秒）。AI 自身の
速度計画を 1 周ぶん積分した基準走行の時間表で換算すると **−0.35 〜 −0.02 秒**に収まる。

難易度は `DIFFICULTY` テーブル（`leaderLapSeconds` 1 つで決まる）。**`hard` をこれ以上
速くできないのは R-1 の帰結**で、速度を 0.95 に落とした車の限界が 71.48 秒/周である。
選択 UI は本フェーズの範囲外（`createRaceState({ difficulty })` の口だけ用意した）。

**検証**: `tests/balance.spec.ts`（15 件）。実測比 0.95・15 秒差・ばらつきの範囲と再現性・
**目標を実際に達成すること**（3 シード × 3 難易度 × 敵車 7 台）・ペースの上下限と変化率・
決定性を固定する。

**残件**: 「普通の走り」のアンカー（74.68 秒）は**人間らしさの模造**であって実測ではない。
人が遊んだベストラップが出たら差し替えて `npm run measure:pace` を流し直すと、
3 難易度がまとめて決まる。
...
要求は `Docs/REFINE_PLAN.md`、計画は `Docs/REFINE_IMPLEMENTATION_PLAN.md`、実測は
`Docs/QUALITY_REVIEW.md` §12。**シミュレーションは 1 つ**という不変条件は変えていない
（`sim/` を触るのは車どうしの衝突と壁の材質だけで、どちらも 4 世代に等しく効く）。

| # | 項目 | 主に触るファイル | アセット再生成 | 状態 |
| --- | --- | --- | --- | --- |
| 11-1 | 第2世代の敵車をパレット替えで色分け | `tools/build-car-sprites.mjs` / `view/shared/car-sprite.ts` | 車スプライト | ✅ |
| 11-2 | スプライトの重ね順を接地線 Y でそろえる | `view/shared/sprite-plane.ts` | 無 | ✅ |
| 11-3 | 車の寸法を sim に置き、車どうしの衝突を入れる | `sim/collision.ts`（新規）/ `sim/vehicle.ts` / `sim/state.ts` | 無 | ✅ |
| 11-4a | 背景オブジェクトとトンネル区間を `sim/` へ移す | `sim/scenery.ts` / `sim/tunnel.ts` | 無 | ✅ |
| 11-4b | 壁の位置をそろえ、材質と大幅減速・衝突音 | `sim/wall.ts`（新規）/ `sim/vehicle.ts` / `audio/sfx.ts` | コースメッシュ 58 本 | ✅ |
| 11-5 | 第3・第4世代の車を実寸にしてカメラを引き直す | `view/shared/car-model.ts` / `view/shared/camera.ts` | 無 | ✅ |
| 11-6 | タイトルの放置で世代を巡回する | `flow/attract.ts`（新規）/ `module.ts` | 無 | ✅ |
| 11-7 | 画面モード（フラットディスプレイ / モアレ） | `view/shared/screen-mode.ts`（新規）/ `bootstrap.ts` / `main.ts` | 無 | ✅ |
| 11-8 | 衝突を入れたあとのバランスの測り直し | `tests/balance.spec.ts` | 無 | ✅ |

#### 11-1 パレット替え（要求 R-1）

実機のスプライトは **1 タイルセット + パレット 16 色**で、色違いの敵車はパレット番号だけを
変えて作った。第2世代のアトラスをその構造で焼く — **8 行すべてが同じインデックス地図**を
共有し、違うのは色だけである（384×256 → 384×1024・固有色 62）。

**どの色が塗装でどの色が共通部品かは、元絵が教えてくれる。** 同梱の `cars.png` は同じ絵を
黄と赤で 2 度描いたもので、2 行はほぼ画素単位で揃っている（13,544 画素が重なり、食い違いは
109 画素）。**2 行で色が変わる画素が塗装・変わらない画素がタイヤ / 窓 / 影 / 灯火**である。
彩度で分けると赤いテールランプが塗装側へ落ちてしまう（実際、灯火は 8 台とも赤いまま残る）。

第1世代は 2 行のまま（同時 25 色の契約）で、出力はバイト一致する。

#### 11-2 重ね順（要求 R-2）

以前は「登録順の逆」に積んでおり、`rivalPlacements()` が遠い順に返すため**遠い車が手前に
出ていた**（完走までの走査で FC は 344 フレーム）。積む順を**接地線（`y + height`）の昇順**
にする。中心や上端で比べると、大きさの違う 2 台が同じ地面に居るときに前後が入れ替わる。

**走査線制限は従来どおり登録順で掛ける。** 制限の優先度（何を消すか）と重なり（何が手前か）は
別の問いで、前者は「自機を必ず残す」ことが要る。ミニマップのマーカーは `depthSorted: false` —
画面に貼られた記号であって世界に居る物ではない。

#### 11-3 車どうしの衝突（要求 R-3）

車体の寸法は **`sim/vehicle.ts` に 1 つだけ**置く（`CAR_LENGTH` 4.2 m / `CAR_WIDTH` 1.95 m）。
衝突判定も 3D モデルの倍率も 2D スプライトの世界寸法もこれを参照するので、**4 世代で車の
実寸が一致する**（`car-size.spec.ts` が固定）。

解くのは**全車を進めたあとに 1 回だけ**である。1 台ずつの `stepVehicle` の中で解くと更新順が
結果を変えてしまう。応答は押し離しと速度の損だけで、跳ね返り係数は 0・ヨーには触れない。
追突は closing 速度の 85 % を後ろから引いて 15 % を前へ渡すので、**合計の運動量は必ず減る**。

#### 11-4 壁の位置と材質（要求 R-4）

3 つの食い違いを閉じた。

1. **シムの壁は描かれている壁より 1.2 m 手前にあった。** コースメッシュが草地の幅を
   `VEHICLE.RUNOFF` にしていて縁石 1.2 m を足し忘れていた。シムが唯一の真実なので絵のほうを
   合わせる（草地 9 → 7.8 m）。位置は `wallLateral()` の 1 つの式から出る
2. **タイヤフェンスに当たり判定が無かった。** 縁から 3 m に立っていて素通りでき、
   「タイヤ壁を通り抜けて草地を走る」ほうが速かった。**壁の内貼りへ移す** — 縁から 3 m の
   まま当たり判定を足すと、その外側（3〜15 m）へ出た車が戻れなくなり 9-1 の保証を壊す
3. **掠りが「大幅減」になっていなかった**（−0.5 m/s ＝ 速度の 1 %）。材質ごとの下限
   （コンクリート 8 %・タイヤ 14 %）を入れる

材質の規則は `sim/wall.ts` の**1 つ**で、タイヤフェンスの配置もそこから引く。
**見えているタイヤ壁と当たり判定が構造的にずれない。** 音は相手ごとに変え、強さは自車速度
ではなく**実際に失った速度**から出す（掠りは小さく、激突は大きく鳴る）。

#### 11-5 車の実寸（要求 R-5）

**素の 3D モデルは 2D スプライトの車の 44 %（幅）／ 37 %（高さ）しかなかった。** 同じ 1 つの
シミュレーションを描いているのに世代で車の実寸が違っていたということで、「第3・第4世代の
自機が小さい」原因はカメラではなくこれだった。倍率はモデルの実測幅から導く（値を手で
書かない）。カメラは `BEHIND` 3.2 → 5.0 m・`HEIGHT` 1.25 → 1.6 m で、車幅は画面の 18 % →
**33 %**（1.8 倍）になる。**目線を上げないと屋根の行と 30 m 先の路面の行がほぼ重なる。**

#### 11-6 アトラクト（要求 R-6）

`flow/attract.ts` が持つ状態は 2 つの数だけ（無操作が何ティック続いたか・いまの世代を何
ティック見せたか）で、「そろそろ次へ」と言うだけの純粋な関数である。送りは
`context.generation.cycle(1)` ＝ **手動の Q / E と同じ経路**なので追加のコストが無い。
デモの作り直しは既に `stepFlow()` にあり、シードは `raceIndex = 0` のままなので
**タイトルを眺めた長さがレースの内容を変えない**という不変条件が保たれる。

#### 11-7 画面モード（要求 R-7）

CRT は 1 本のシェーダ ＋ 7 つの uniform で、ここで作るのは**上書きだけ**である
（`crtOverride` は毎フレーム評価されるので作り直しは要らない）。
**フラットディスプレイ**は樽型歪みだけを 0 に、**モアレ OFF** は蛍光体マスクと走査線の
両方を 0 にする（どちらも出力画素の周期を持つ模様）。**既定は「いまの見え方」**で、
上書きは空 — 初回起動の絵が変わらないことがこの機能の受け入れ条件でもある。

**検証**: 新規 6 本（`car-palette` / `sprite-order` / `car-contact` / `wall-material` /
`car-size` / `attract` / `screen-mode`）と既存 8 本の手当てで **516 件**。実画面の確認は
`Docs/QUALITY_REVIEW.md` §12。

### フェーズ 12 — 車モデルの入れ替え（第3・第4世代）✅ 実装済み

要求は `Docs/REPLACE_DATA.md`、計画と実測は `Docs/REPLACE_DATA_PLAN.md`。**シミュレーションは
1 つ**という不変条件は変えていない（`sim/` は 1 行も触っていない。車の実寸を決めるのは
`VEHICLE.CAR_LENGTH` / `CAR_WIDTH` の側であって、モデルの側ではない）。

| 段 | 項目 | 主に触るファイル | 状態 |
| --- | --- | --- | --- |
| 12-1 | baseline JPEG デコーダ | `tools/lib/jpeg.mjs`（新規） | ✅ |
| 12-2 | 変換器に正規化を焼き込み、テクスチャも焼き直す | `tools/prepare-cars.mjs` / `public/assets/car-conversion.json` | ✅ |
| 12-3 | bounds の写し替えと塗装の目標明度 | `view/shared/car-model.ts` / `tools/build-car-paint.mjs` | ✅ |
| 12-4 | 正規化を固定するテスト | `tests/car-conversion.spec.ts`（新規） | ✅ |
| 12-5 | 実画面での確認 | 無（`Docs/screenshots/newcar-*.png`） | ✅ |
| 12-6 | 文書の追随 | `data/README.md` / 本書 / `README.md` | ✅ |
| 12-7 | テールランプとタイヤの回転 | `tools/prepare-cars.mjs` / `tools/build-car-lamps.mjs`（新規） / `view/shared/car-model.ts` / `view/gen3-ps1.ts` / `view/gen4-ps2.ts` | ✅ |

#### 12-1〜12-2 正規化は変換器へ焼き込む（要求 R-1〜R-3）

新しい変換元（Tripo3D 出力）は**姿勢も原点も寸法も現行と違った** — 前方が `+Z` で Y 回りに
32.4° / 34.4° のヨーが付き、原点は底面（`Y = 0`）、全長 / 車幅の比は 1.95（現行 2.17）。

実行時の `TransformCommand` は `rotationY` と等方 scale しか持たないので、この差はすべて変換器で
吸収するしかない。node の TRS へ逃がすと accessor の min/max が実際の姿勢と食い違い、
`CAR_MODELS.bounds` の意味が壊れる。**定数は記録の `normalize` に置き、ツールは探索しない。**

寸法だけは相似にしなかった（D-4）。車幅で合わせて相似に縮めると全長が 3.81 m / 3.79 m になり、
衝突判定の 4.2 m と 0.4 m 食い違う ＝ **見た目より先に当たる車**になる。左右・上下を 1 つの倍率、
前後を別の倍率で正規化して、実行時に 4.200 m × 1.950 m へ載せた。前後の引き伸ばしは 1.10 / 1.11 倍で、
実画面で車輪の楕円化は見えない。

テクスチャも変換元から焼けるようになった。新しい base color は JPEG しか無く、
`tools/lib/png.mjs` が zlib だけで PNG を自前実装しているのと同じ作法で、baseline JPEG の
最小デコーダを置いた（依存は増やしていない）。

#### 12-3 塗装の目標明度（要求 R-5）

**色替えの仕組みは変えていない** — 無彩色テクスチャ 1 枚 × `MeshCommand.color` の乗算のまま。
動かしたのは `PAINT_TARGET` という数値 1 つだけである。新しいリバリーは黄で塗装部の V の平均が
0.727 / 0.673 と高く、現行の 0.82 では塗装面の 6 割が 255 へ張り付いて曲面の陰影が消えた。
0.70 で白飛びは 0.0 % / 2.9 %、明るさ（上位 25 %）は 187 / 207 と現行（赤・203 / 255）と同等になる。

#### 12-4〜12-5 検証

**既存テストを 1 つも書き換えずに通ることが、この設計の受け入れ条件**だった（前方 `-X` の維持・
正規化の焼き込み・車幅で合わせる倍率）。実際に 516 件が無修正で通り、新規
`car-conversion.spec.ts` 17 件を足して **533 件**。新規テストはヨーを 0.5° ずらして焼き直す
反証を実際に走らせ、3 件が落ちることを確かめてある。

実画面（`Docs/screenshots/newcar-*.png`・`Docs/screenshots/` は gitignore なのでローカルのみ）:
第3・第4世代とも自機が進行方向を向いて接地し、8 台がエントラント色で描き分けられている
（R-4 / R-5）。前走車を衝突距離ちょうど 4.2 m 前へ置いた絵ではバンパーが接し、隙間も重なりも無い。

**副作用**: 第4世代の車が 13,618 → 7,997 tri になり、走行中 1 フレームは 136,900 → 91,932 tri へ
減った。初回ロードは 6.94 → 6.75 MB。第1・第2世代のスプライトは対象外である（D-10 — 同梱
`cars.png` 由来で 3D モデルとは別のデザイン。合わせるなら新モデルからアトラスを焼く別の作業になる）。

#### 12-7 テールランプとタイヤの回転（第3・第4世代）

**エンジンの制約が設計をそのまま決めた 2 つ**である。

| したいこと | 素直な手 | なぜ使えないか |
| --- | --- | --- |
| 車軸まわりにタイヤを回す | `TransformCommand` の回転 | `rotationY` しか無い（`render/frame.d.ts`） |
| 〃 | `SkinnedMeshCommand` のクリップ | スキン経路がマテリアルを読まない（`uEnvironmentStrength: 0`・`uAmbient` は 0.45 固定）。第4世代でタイヤだけ映り込みも陰影も車体と別系統になる |
| テクスチャで灯火を光らせる | `MaterialCommand.emissiveTexture` | 型にあるだけで**レンダラーが読んでいない**（`uBaseColor` / `uTopColor` しか無い） |
| テクスチャの赤を灯火に使う | 元の base color | 塗装テクスチャが無彩色へ均す（`CAR_PAINT`）。第4世代の赤い画素は左右対称にも並んでいない（実測 23 三角形が側面にも散る） |

したがって**どちらも「焼いた形」で持つ**。

- **タイヤ**: 変換器が車体から連結成分としてタイヤを切り出し（第3世代 4 個・第4世代 6 個 ＋ 車軸の
  センターキャップ 4 個）、位相を 8 段階に焼いた `car_wheels_<位相>.glb` を出す。実行時は走った
  距離 `s` から位相を選んで `MeshCommand.asset` を差し替えるだけで、**シムには何も足していない**。
  回転は変換元の空間（正規化で前後 1.10 倍に伸びる前）で掛ける — そのまま回すと楕円が首を振り、
  8 位相の bounds のぶれが 0.0160 / 0.0239 に対し、戻して回すと 0.0050 / 0.0110 に収まる。
  分割は可逆で、車体 ＋ 位相 0 が分割前の三角形・頂点にちょうど戻る（`geometry` の記録はこれまでどおり
  分割前の全体を指し、`CAR_MODELS.bounds` の意味も接地の意味も変わらない）。
- **灯火**: `tools/build-car-lamps.mjs` が車体の後ろ向きの面から位置を実測し、左右 1 本ずつの帯を焼く。
  **列ごとに車体表面の X を測り直す** — 1 つの X に平らな板を置くと外端が車体の輪郭からはみ出し、
  路面の上に赤い板が浮く（実画面で確認して直した）。板が引くのは塗装テクスチャの明るい無彩色
  テクセル 1 点だけなので、テクスチャは 1 枚も増えず、灯火の色は `MeshCommand.color` の乗算 1 つで決まる。
  常時は暗い赤、ブレーキで明るい赤へ混ぜる（`DisplayCar.brakeInput` を 1 本足した）。

世代差もここに出る。第3世代は深度バッファが無いので**車輪 → 車体 → 灯火の順**に同じスロット 9 へ積む
（先に車輪を置くと、向こう側の車輪とフェンダーに隠れる部分を車体が上書きしてくれる）。第4世代は
順序の指定を 1 つも持たない。灯火はどちらも「カメラが車の後ろにいるとき」だけ積む — 板は後ろ向きの
一枚面なので前から見れば背面カリングで消えるが、**絵は変わらずドローコールだけが残る**ためで、
第4世代の予算（§6.3・240 コール）を 240 ちょうどから 233 へ戻すのがこの 1 行である。

**副作用**: 1 台あたりのドローコールが第3世代 1 → 3・第4世代 2 → 4（影を含む）に増え、
三角形は 1 台 7,997 → 8,013（灯火 16 枚ぶん。車体と車輪の合計は分割前と同じ）。車のアセットは
車体が縮んだぶんを差し引いて **第3世代 +100 KB / 第4世代 +857 KB**（10 ファイルずつ）。
テストは 533 → 563 件（`tests/car-parts.spec.ts` 新規 29 件 ＋ 塗装 1 件）。
