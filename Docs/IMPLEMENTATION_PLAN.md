...
| `public/assets/gen{1,2,3,4}/sprites/scenery.png` | `build-scenery-sprites.mjs` | 看板・木・タイヤフェンス（3 セル）。擬似3D 世代はスクリーン空間、3D 世代はビルボード | 8-6 |
| `public/assets/gen4/models/tyre-wall.glb` | `build-scenery-mesh.mjs` | タイヤフェンス（第4世代のみ）。UV は路面アトラスの壁の帯を引く | 8-6 |
...
| `scenery.spec.ts` | 4 世代が同じ `sceneryObjects()` を読み、看板の `s` が世代間で完全一致する。FC は 8px 丸め済み・`hardwareBlend` 0 件・看板 1 種だけ。第3世代の木は車と同じスロット 9、第4世代は描画順の指定を持たない（8-6） |
...
**結果**: §6.1 の全基準を実画面で計測し、`Docs/QUALITY_REVIEW.md` に記録した。8-1〜8-8 はすべて実施済み。実測の要点は次のとおり。
...
- **初回ロード**: 90 ファイル 4.95 MB（フェーズ 5 の 4.41 MB から、タコメーター・内装・背景アトラス・壁を焼き込んだコースメッシュのぶん増えた）
...
**残件**（`Docs/QUALITY_REVIEW.md` §8）: 音を耳で確認していないこと・第1世代の遠景が BG の 2bpp（タイルあたり 4 色）に収まらないこと・第1世代の走行 HUD が雲の上で読みにくいこと・第4世代の金網フェンス（8-6 の表にあるが未実装）・リザルトの表が順位とベストラップのみであること。
...
5. 下の改良項目 8-1〜8-8 を実施し、それぞれの検証を通す。**着手順は 8-8 → 8-7 → 8-1 → 8-4 → 8-3 → 8-5 → 8-2 → 8-6**（表示と入力のように小さく閉じる物を先に、アセットの再生成を伴う物を後に置く）
...
| 8-6 | 背景オブジェクト | 全 | `view/shared/scenery*.ts` / `tools/build-track-mesh.mjs` / `tools/build-scenery-*.mjs` | **有** | ✅（金網フェンスのみ見送り） |
...
| 値 | 現在 | 変更後 | 理由 |
| --- | --- | --- | --- |
| `BEHIND` | 6.5 m | **4.4 m** | 車体を画面の主役にする |
| `HEIGHT` | 2.2 m | **1.55 m** | 見下ろしを浅くして速度感を出す |
| `LOOK_AHEAD` | 12 m | **9 m** | 近づけたぶん注視点も引き寄せる（遠いままだと車が画面下端へ落ちる） |
| `TARGET_HEIGHT` | 0.9 m | **0.75 m** | 同上 |
| `BEHIND_STRETCH` | 1.6 m | **1.1 m** | 最高速でも 5.5 m に収める |
...
**検証**: `Docs/screenshots/` の前後比較。第4世代で映り込みが車の向きに応じて流れることが**静止画 2 枚で読み取れる**ことを合格条件にする。
